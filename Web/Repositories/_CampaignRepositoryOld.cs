...
namespace Maximiz.Repositories
{
    ...
    /// </summary>
    public class _CampaignRepositoryOld
    {
        ...
        public _CampaignRepositoryOld(IDatabaseProvider databaseProvider)
        {
        ...
        async Task<IEnumerable<Campaign>> GetAll(ColumnCampaign column, Order order)
        {
            using (IDbConnection connection = _databaseProvider.GetConnectionScope())
            {
                var columnString = new QueryTranslator().Convert(column);
                var orderString = new QueryTranslator().Convert(order);
                try
                {
        ...
        /// <returns>Retrieved list of campaigns</returns>
        async Task<IEnumerable<Campaign>> Search(string query)
        {
            try
            {
